assign("moranbi.cluster",
   function(x, y, listw, zero.policy = NULL, shp, significant=T, alternative="two.sided", ...){
  require(RColorBrewer)
  require(spdep)
mor.dat <- localmoran.bi(x, y, listw, zero.policy=zero.policy, alternative=alternative, ...)    
wy<-lag.listw(listw, y)
lag.z<-scale(wy, center=T, scale=T)
...
paste("Low-High  ","(",length(mor.dat1$cluster[mor.dat1$cluster=="LH"]),")",sep="",collapse=""), paste("High-Low  ","(",length(mor.dat1$cluster[mor.dat1$cluster=="HL"]),")",sep="",collapse=""), paste("Neighborless  ","(",length(mor.dat1$cluster[mor.dat1$cluster=="NA"]),")",sep="",collapse="")), fill=c(cols[3],cols[1],cols[5],cols[4],cols[2],cols[6]), title = "BiLISA Cluster Map", bty="n", cex=1.2, y.intersp=0.8)
