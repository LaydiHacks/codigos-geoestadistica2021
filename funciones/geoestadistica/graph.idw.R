assign("graph.idw",
       function(formula, data, locations, np, p.dmax, P.T=NULL, nmax=Inf, nmin=0, progress=F, ...)
       {
         if (!is.logical(P.T) & !is.null(P.T))
           stop(paste("P.T must be logical"))
         Opt <- optimize(idw.cv, c(0,p.dmax), formula=formula, locations=locations, data=data, nmax=nmax, nmin=nmin, 
                         progress=F)
         ...
         Table <- Table0[orden, ]
         p <- plot(Table, lty = 1, ylab = "RMSPE", col = 4, xlab = "p", lwd=1.5, type = "l")      # %<a-%
         Optim <- Table[which.min(Table[, 2]), ]
         legend(locator(1),legend=c(list(paste("Optimal p =  ",round(Optim$P,4)),paste("RMSPE   = ",round(Optim$RMSPE,4)))), box.col=F, cex=0.8)
         cat("Optimal p IDW: ", "\n", "p  = ", Optim$P, "\n", "RMSPE   = ", Optim$RMSPE, "\n")
