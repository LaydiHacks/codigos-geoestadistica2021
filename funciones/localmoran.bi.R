...
function (x, y, listw, zero.policy = NULL, na.action = na.fail, 
    alternative = "greater", p.adjust.method = "none", mlvar = TRUE, adjust.x = FALSE,
    spChk = NULL) 
{
    ...
    res[, 1] <- (z/sqrt(s2*sy2))*lyz
    Wi <- sapply(listw$weights, sum)
    res[, 2] <- -(Wi*cor(x,y))/(n - 1)
    if (mlvar) {
        if (adjust.x) {
            b2 <- (sum((z[nc]^2)*(yz[nc]^2), na.rm = NAOK)/sum(nc))/(s2*sy2)
        }
        else {
            b2 <- (sum((z^2)*(yz^2), na.rm = NAOK)/n)/(s2*sy2)
        }
    }
    else {
        if (adjust.x) {
            b2 <- (sum((z[nc]^2)*(yz[nc]^2), na.rm = NAOK)/(sum(nc) - 1))/(s2*sy2)
        }
        else {
            b2 <- (sum((z^2)*(yz^2), na.rm = NAOK)/(n - 1))/(s2*sy2)
        }
    }
    A <- (n - b2)/(n - 1)
    B <- (2 * b2 - n)/((n - 1) * (n - 2))
