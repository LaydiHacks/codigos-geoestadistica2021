spcorrelogram.bi <- function (neighbours, varX, varY, listw, order = 1, method = "I", style = "W", alternative = "greater",
    randomisation = TRUE, zero.policy = NULL, spChk = NULL)
{
    ...
    obj <- list(res = res, method = method, cardnos = cardnos, 
        varX = deparse(substitute(varX)),varY = deparse(substitute(varY)),alternative=alternative)
    class(obj) <- "spcorbi"
    obj
}
