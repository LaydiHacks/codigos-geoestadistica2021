...
points.geodata(preci.geoR, x.leg=3, y.leg=5, main=c("Gr?fico de Intensidades", "de Precipitaci?n"), col.main=3, pt.div="quintile")
...
points.geodata(preci.geoR, x.leg=3, y.leg=5, main=c("Gr?fico de Intensidades", "de Precipitaci?n"), col.main=3, pt.div="quintile", add.to.plot = TRUE, panel.first = grid())          
                                   
# Valores de precipitaci?n, desplazados para no superponerlos con los puntos
attach(preci)
text(x+0.12,y+0.12,prec,cex=0.6)
# 4 gr?ficas: configuraci?n puntos en R2, perfiles E-W y S-N, e histograma de la funci?n de densidad, utiles para ver la distribuci?n espacial de los puntos, an?lizar tendencia en las direcciones E-W y S-N y analizar normalidad
plot(preci.geoR) 

# Para hacer gr?ficos en 3 dimensiones                                    
library(scatterplot3d)               
s3d<-scatterplot3d(x, y, prec, angle=45, main="Gr?fico de Precipitaci?n en 3D", col.main=4, xlab="Coordenada X", ylab="Coordenada Y", zlab="Precipitaci?n")
# Adicionando un plano de regresi?n al gr?fico anterior
my.lm <- lm(prec ~ x )
...
qqnorm(prec, ylab="prec (mm)", xlab="Cuantiles te?ricos")
qqline(prec)                                                        #Agregarle al Q-Q Plot la l?nea media
boxplot(prec, main="BOX-PLOT", notch=F, horizontal=T, xlab="Precipitaci?n (mm)")
points(mean(prec), y=1, pch=1, cex=2)
par(mfrow=c(nrow=1, ncol=1))

# Pruebas de normalidad de Shapiro-Wilk y Kolmogorov-Smirnov
# Verificaci?n del supuesto de normalidad para los m?todos lineales de kriging.
      # Opciones para tranformar:
      # boxcox.fit(geoR), para un 'vector' de datos
      # boxcox(), para un objeto de tipo 'geodata' con GeoR

shapiro.test(prec) #Esta variable ya sigue una distribuci�n normal
n.q <- (prec-mean(prec))/sd(prec)            #Funci�n prec
...
# Estimaci?n del semivariograma:
detach(preci)
# La instrucci?n "vgm()", muestra las funciones de variograma disponibles en el programa geoR, se debe haber cargado el paquete gstat previamente.
vgm()
# Estimador cl?sico
...
plot(prec.cl1, pch=20, main="Semivariograma Experimental Cl?sico")
grid()
...
#Calcula el variograma de la muestra o residual (en el caso que se d? un modelo de tendencia)
                     
v <- variogram(prec~1, loc=~x+y, cutoff=1, cressie = F, data=preci, cloud=TRUE)      

           #########################################################################################
           ##    Kriging Simple: Funcionamiento Predicci?n un punto y estimaci?n de su varianza   ##
           #########################################################################################

# En este caso la funci?n de covarianza se debe construir a partir del vector de errores obtenidos de la diferencia entre las 
# observaciones y la media de la variable regionalizada poblacional, asumiendo previamente estacionariedad
# Para el ejemplo los par?metros de la funci?n de covarianza son tomados del programa ArcGIS
# Funcionamiento con un vecindario de tama?o 5

#Este ejemplo se ve en el Excel 
# https://docs.google.com/spreadsheets/d/15qw3LAVmH0d8jNIJPysqZwzinwNMQnE_/edit#gid=1556490607 


#Tomar los datos de la precipitaci�n 
xy <- preci[,2:3]  
z <- preci$prec
...
proc.time()
          ...
          ##########################################################################################
          ##  Kriging Ordinario: Funcionamiento Predicci?n un punto y estimaci?n de su varianza   ##
          ##########################################################################################
...
krige(z~1, pts, SpatialPoints(So), vgm(112.33, "Sph", 4.3441), nmax=5)  #Krigiado con el metodo como IW lo que hacemos es quitar el variograma   
proc.time()


                             ###################################################
                             ######### Validaci?n Cruzada KO y KS ##############
                             ###################################################

semiv.z <- variogram(z~1, ~x+y, data=preci, uvec=seq(0,7,1))            # Igual a "cressie=F" m?todo cl?sico
plot(semiv.z, pl=T, ylab="Semivarianza", xlab="Distancia (m)")
sem.esf.sim <- vgm(112.33, "Sph", 4.3441)                               # Esf?rico para kriging Simple
...
KO.esf.cv.z <- krige.cv(prec~1, ~x+y, precipi, sem.fit.esf.ord, nmin=0, nmax=5)  #Validación cruzada para el caso que no hay tendencia  
KS.esf.cv.z <- krige.cv(prec~1, ~ x+y, precipi, sem.fit.esf.sim, beta=mu, nmin=0, nmax=5)

# Correr primero la función criteria.cv

resultados.cv.z <- rbind(criteria.cv(KO.esf.cv.z), criteria.cv(KS.esf.cv.z))
rownames(resultados.cv.z) <- c("KO.esf.cv.z", "KS.esf.cv.z")
resultados.cv.z


            ########################################################################################
            ## Generaci?n de los mapas de pron?stico y de errores para Kriging Simple y Ordinario ##
            ########################################################################################
...
write.asciigrid(pron.pts.ko, "ko.asc")
          ...
          ##########################################################################################
          ##  Kriging Universal: Funcionamiento Predicci?n un punto y estimaci?n de su varianza   ##
          ##########################################################################################

# En este caso la funci?n de covarianza se debe construir a partir del vector de errores obtenidos de la diferencia entre las observaciones y los pron?sticos obtenidos de un modelo de deriva (o tendencia, es decir se debe correr una regresi?n de un polinomio de orden 1, 2 o 3 "depende de la cantidad de puntos que se disponga", y en esta regresi?n la variable regionalizada Z es la variable respuesta y las variables asociadas a las coordenadas X y Y son las variables independientes), asumiendo previamente no estacionariedad fuerte, pero si estacionariedad intrinseca o d?bil.
# Para el ejemplo los par?metros de la funci?n de covarianza son tomados del programa ArcGIS, esto solo con el objetivo de realizar la comparaci?n entre R y ArcGIS.
...
# A partir del orden 3 tendr?amos m?s par?metros que ecuaciones, o si consideramos menos observaciones en el vecindario con un orden 2 tendriamos varianzas negativas.
...
resultados.cv1.z
# Se recuerda que 0 <= R2 <= 1, debido a la validaci?n cruzada en este caso fue menor de 0, 
# dicho criterio no se aplica para la decisi?n. Adem?s se evidencia que entre m?s se aleja del intervalo [0,1], 
# mayor es el error.



          ###########################################################################################
          ## Generaci?n de los mapas de pron?stico y de errores para Kriging Universal Orden 1 y 2 ##
          ###########################################################################################
...
##########################################################################################
##  Kriging Indicador: Funcionamiento Predicci?n un punto y estimaci?n de su varianza   ##
##########################################################################################
detach(preci)
...
spplot(idw.int2[1], col.regions=bpy.colors(100), scales = list(draw =T), xlab ="Este (m)", ylab = "Norte (m)", main = "Interpolaciones de Distancia Inversa\n Ponderada de la Precipitaci?n (P=2)")
                           ...
                           ####################################################################
                           ########       OPTIMIZACI?N PARA SELECCIONAR P EN IDW     ##########
                           ####################################################################
...
plot(c(1,2,P$minimum,3,4,5),RMSPE, main="Gr?fico de optimizaci?n del Par?metro (P)\n Distancia Inversa Ponderada", ylab="RMSPE", xlab="p ?ptimo = 2.96997", type="l")
...
plot(c(0.5, 1, 1.5, 2, 2.5, P$minimum, 3, 3.5, 4, 4.5, 5, 5.5, 6),RMSPE, main="Gr?fico de optimizaci?n del Par?metro (P)\n Distancia Inversa Ponderada", ylab="RMSPE", xlab="p ?ptimo = 2.96997", type="l")
...
load("D:/CARLOS/Doctorado/Investigaci?n 2008-2009/Dise?o e Implementaci?n de Software Estad?stico/Resultados/Metodos Kriging/Final/Envio/Kriging y FBR")
# save.image("D:/CARLOS/Doctorado/Investigaci?n 2008-2009/Dise?o e Implementaci?n de Software Estad?stico/Resultados/Metodos Kriging/Final/Envio/Kriging y FBR")